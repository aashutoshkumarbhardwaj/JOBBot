...
SEEN_FILE = BASE_DIR / "seen_jobs_sakshi.json"
...
def fetch_lever(slug):
    ...
    jobs = r.json()
    return [
        (j["id"], j.get("text", "Untitled role"), j.get("hostedUrl", ""), j.get("descriptionPlain", ""))
        for j in jobs
    ]
...
def fetch_aggregated_jobs():
    ...
    search_terms = ["Research", "Quality Assurance", "Biotech", "Food Tech"]
            ...
            if jobs_df is not None and not jobs_df.empty:
                for _, row in jobs_df.iterrows():
                    ...
                    if not url or url == 'nan':
                        continue
                    jobs_found.append((url, company, title, url, description))
...
def main():
    ...
    for company in companies:
        ...
        for job_id, title, url, description in jobs:
    ...
    for job_id, company_name, title, url, description in scraped_jobs:
